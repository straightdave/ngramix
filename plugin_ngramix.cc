...
/* The improved part has no other copyright but The Authorship 
  which belongs to 'straightdave' (straightdave.github.io) 2016 */
        ...
        bool_info)
{
  ...
  while (next < end) {
    ...
    if (n_chars == ngram_token_size) {
      ...
      /* Move a char forward */
      int temp_char_len = my_mbcharlen_ptr(cs, start, end);
      if (temp_char_len != 1) {
        start += temp_char_len;
      }
      else {
...
static MYSQL_SYSVAR_INT(token_size, ngram_token_size,
  PLUGIN_VAR_READONLY,
  "InnoDB ngraMIX full text plugin parser token size in characters",
  NULL, NULL, 2, 1, 10, 0);
...
mysql_declare_plugin(ngram_parser)
{
  ...
  "ngramix",      /*!< name */
  "straightdave",      /*!< author */
  "Ngramix Full-Text Parser", /*!< description*/
  PLUGIN_LICENSE_GPL,
